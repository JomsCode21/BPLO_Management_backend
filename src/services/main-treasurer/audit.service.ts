...
type GetMainTreasurerWorkflowAuditParams = {
  page: number;
  limit: number;
  permit?: string;
  source?: string;
...
type WorkflowAuditCursorRow = {
  ...
  statusSource?: string;
  adminRemark?: string;
  paymentAssessmentSnapshot?: unknown[];
  createdAt?: Date | null;
  submittedAt?: Date | null;
};

type MainTreasurerWorkflowAuditEventRow = {
  _id: string;
...
type PaymentBreakdownItem =
  MainTreasurerWorkflowAuditEventRow["paymentBreakdown"][number];

const MAIN_TREASURER_ALLOWED_STATUSES = new Set([
  "payment_paid",
  "payment_confirmed",
]);

const MAIN_TREASURER_ALLOWED_SOURCES = ["system", "treasurer"];
...
const resolveTreasurerActor = (row: any): string => {
...
const buildSearchHaystack = (row: MainTreasurerWorkflowAuditEventRow) =>
  [
...
const collectWorkflowStatusIds = async (params: {
  ...
  const cursor = OwnerApplicationStatus.find(params.match)
    .select(["_id", "application", "statusSource", "createdAt"].join(" "))
...
const hydrateMainTreasurerWorkflowAuditEvents = async (
  statusIds: string[],
): Promise<MainTreasurerWorkflowAuditEventRow[]> => {
  ...
  return rows
    .map((row: any) => {
      ...
      const snapshot = Array.isArray(row?.paymentAssessmentSnapshot)
        ...
        : [];
      const assessments =
        snapshot.length > 0 ? snapshot : applicationAssessments;
      const paymentBreakdown: PaymentBreakdownItem[] = assessments
        .map((assessment: any): PaymentBreakdownItem => ({
          departmentName: normalizeText(assessment?.departmentName) || "Department",
          amount: Number(assessment?.totalAmount ?? 0),
          paymentStatus:
            normalizeText(assessment?.paymentStatus).toLowerCase() === "paid"
              ? ("paid" as const)
              : ("pending" as const),
        }))
        .filter((item: PaymentBreakdownItem) => item.amount >= 0);
      const totalAmountToSettle = assessments.reduce(
        (sum: number, assessment: any) =>
          sum + Number(assessment?.totalAmount ?? 0),
        0,
      );
      const computedPaidAmount = assessments.reduce(
        (sum: number, assessment: any) =>
          normalizeText(assessment?.paymentStatus).toLowerCase() === "paid"
            ? sum + Number(assessment?.totalAmount ?? 0)
            : sum,
      ...
      const amountPaid = isPaidEvent ? totalAmountToSettle : computedPaidAmount;
      const paidTimes = assessments
        .filter(
          (assessment: any) =>
            normalizeText(assessment?.paymentStatus).toLowerCase() === "paid" &&
            assessment?.statusUpdatedAt,
        )
        .map((assessment: any) => new Date(assessment.statusUpdatedAt).getTime())
        .filter((value: number) => !Number.isNaN(value));
      const paidAt =
        paidTimes.length > 0 ? new Date(Math.max(...paidTimes)) : null;
      ...
      return {
        ...
        actorName: resolveTreasurerActor(row),
...
const getMainTreasurerWorkflowAuditRowsForScope = async (
  params: Omit<GetMainTreasurerWorkflowAuditParams, "page" | "limit">,
) => {
  const match: Record<string, any> = {
    deletedAt: null,
    status: { $in: [...MAIN_TREASURER_ALLOWED_STATUSES] },
  };

  const source = normalizeText(params.source).toLowerCase();
  const normalizedSource =
    source && MAIN_TREASURER_ALLOWED_SOURCES.includes(source) ? source : "";
  ...
  const hydratedRows = await hydrateMainTreasurerWorkflowAuditEvents(statusIds);
  return shouldFilterBySearch
    ? hydratedRows.filter((row) => buildSearchHaystack(row).includes(search))
    : hydratedRows;
};

export const getMainTreasurerWorkflowAuditEventsS = async (
  params: GetMainTreasurerWorkflowAuditParams,
) => {
  ...
  const permitsPromise = OwnerApplicationStatus.distinct("permitName", match).then(
    (values) => {
  ...
  const filteredRows = await getMainTreasurerWorkflowAuditRowsForScope(params);
...
export const downloadMainTreasurerWorkflowAuditPdfS = async (
  params: Omit<GetMainTreasurerWorkflowAuditParams, "page" | "limit">,
) => {
  const rows = await getMainTreasurerWorkflowAuditRowsForScope(params);
  ...
  const drawHeader = () => {
    page.drawText("Main Treasurer Workflow History", {
      x: left,
      y,
      size: 15,
      font: bold,
    ...
    if (normalizeText(params.search)) scopeTags.push(`Search: ${normalizeText(params.search)}`);
    if (normalizeText(params.permit)) scopeTags.push(`Permit: ${normalizeText(params.permit)}`);
    if (normalizeText(params.dateFrom)) scopeTags.push(`From: ${normalizeText(params.dateFrom)}`);
    if (normalizeText(params.dateTo)) scopeTags.push(`To: ${normalizeText(params.dateTo)}`);
    page.drawText(
  ...
  for (const row of rows) {
    ...
    page.drawText(truncateText(formatCurrencyForPdf(row.totalAmountToSettle), 18), {
      x: 500,
      y,
      size: 9,
      font,
